...
public class Robot extends TimedRobot {  
  ...
  //E4T Encoder converting one tick into feet (1/ticks per rotation * gear reduction/1 * wheel diameter * pi/wheeldiameter * 2
  private final double kDriveTick2Feet = 1.0/4096 * 75 * 6 * Math.PI/12;
  ...
  public void robotInit() {
    ...
    //Sets encoder for the master motors,sets loop to closed, sets a timeout of 10ms
    Robot.driveTrain.frontLeftMotor.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0, 10);
    Robot.driveTrain.frontRightMotor.configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0, 10);

    //Makes encoder values of both sides of the drive train so that forward is positive
    Robot.driveTrain.frontLeftMotor.setSensorPhase(false);
    Robot.driveTrain.frontRightMotor.setSensorPhase(true);

    //Sets the encoder values to 0
    Robot.driveTrain.frontLeftMotor.setSelectedSensorPosition(0, 0, 10);
    Robot.driveTrain.frontRightMotor.setSelectedSensorPosition(0, 0, 10);
  ...
  public void robotPeriodic(){
    ...
    SmartDashboard.putNumber("Left Drive Encoder", Robot.driveTrain.frontLeftMotor.getSelectedSensorPosition() * kDriveTick2Feet);
    SmartDashboard.putNumber("Right Drive Encoder", Robot.driveTrain.frontRightMotor.getSelectedSensorPosition() * kDriveTick2Feet);
